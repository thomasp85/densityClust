...
#'
#' @param k The number of peaks to find based on highest \code{gamma} (\code{= rho * delta}) values. Specifying
#'   this value overides any values of \code{rho} or \code{delta}.
#'
#' @param plot Logical. Should a decision plot be shown after cluster detection
#'
#' @export
#' @importFrom graphics plot locator
findClusters.densityCluster <- function(x, rho = NULL, delta = NULL, k = NULL, plot = FALSE, ...) {
  # Detect peaks
  if(!is.null(k)) {
      # Find top k peaks based on gamma
      x.rho <- x$rho
      x.delta <- x$delta
      if(is.null(names(x.rho)) | is.null(names(x.delta))) {
        names(x.rho) <- names(x.delta) <- 1:length(x.rho)
      }
      gamma <- sort(x.rho * x.delta, decreasing = TRUE)
      top.k <- names(gamma)[1:k]
      rho <- min(x.rho[top.k])
      delta <- min(x.delta[top.k])
      rho.next <- max(x.rho[x.rho < rho])
      delta.next <- max(x.delta[x.delta < delta])
      rho <- mean(c(rho, rho.next))
      delta <- mean(c(delta, delta.next))
    } else if(missing(rho) || missing(delta)) {
      # Identify peaks based on user input from graph
      x$peaks <- NA
      plot(x)
    ...
    # sort clusters based on gamma
    gamma <- x$rho* x$delta
    peak.order <- order(gamma[x$peaks], decreasing = TRUE)
    clust.names <- names(x$clusters)
    x$clusters <- peak.order[x$clusters]
    names(x$clusters) <- clust.names
    x$peaks <- x$peaks[peak.order]
